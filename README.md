condition
// read this https://stackoverflow.com/questions/13358018/what-is-the-dart-expando-feature-about-what-does-it-do not expando but typedef somefunction(param param) i used different syntax.
    // TODO: #WJ24DN1C67PQ!WXM [Edit: NO! BUT YOU CAN UTILIZE COMPUTE FUNCTION WITH HOPE IT WILL SUDDENLY WORK WITH WEBASSEMBLY AND WITH THIS METHOD OR NOT THIS CLASS WILL BE UPDATED:  ConditionIsolateNoParallelFunctionCalls class with supporting classes, types exists] INVESTIGATE COMPREHENSIVELY MULTIPLATFORM ISOLATES/JS WEBWORKERS ESPECIALLY WITH LIMITATIONS TO WEBWORKERS TO DECIDE WHETHER OR NOT OT USE IT FOR BELOW STUFF (FALLBACK SOLUTION: THE EVENT LOOP), YOU CAN CREATE ONE CLASS FOR A PATTERN OF ACTION BELOW. SOLUTION - CREATE SIMPLE CLASS FOR NOW USING THE MUltiplatform COMPUTE FUNCTION (works the same everywhere except that web uses the event loop and native uses isolate) - described here https://github.com/brilliapps/dart_tips_solutions_informal/blob/main/README.md In the future the new class will use something new for web or also in the future if it will compile to WASM it is expected to automatically run in webworkers in WASM version, but compiled in compatible "old" way will just run on the event loop. So all teh code should be written in not much locking the event loop way but not to bother too much, just to bear in mind. The class would also gradually be improved that isolates might be run instead of the compute method for native. NO NEED FOR IT NOW.
    // TODO: [Edit: NO! ConditionIsolateNoParallelFunctionCalls class with supporting singleton and non-singleton, supporting classes, types exists, it seems to have the full, full capability of doing all sort of stuff like this from just the current event loop, optional isolates, but also fully synchronical function calls, returning custom class Futures with values of finished futures for fully sync context, also it was too late to consider using a newly discovered Pool class of the pool package which seems to be less capable especially in the context of this library, but stil worth analysing, using elsewhere] BASED ON LAST EXPERIENCE OF COMMONLY REPEATED PATTERNS OF CODE AND BEHAVIOUR MAKE A STANDARIZED CLASS with one method that sync body right now is being executed or not, it's async aspect is being executed or not, returning future or null, null when f.e. body of last call is being executed. Check out if possible: There must be an implementation for both isolate and js workers or none of them (only event loop). There might be as convenience one async future for async aspect and sync future of sync aspect. Because this is not recomended to use synchronous future/completer there is an normal future with current value - which basically means that when the future is created it already has value and you don't need to wait async/await and use it in the code.    // ANY TODO IN THE METHODS PROBABLY AS WELL AS SOME OTHER TIMINGS WITH NOT HAVING TWO THE SAME METHOD CALLS AT THE SAME TIME.
...
-------------------
flutter_widget_from_html_core, packages to test? super_editor, appflowy_editor
...
When using this library in a custom way, please bear in mind that it was designed to work mainly on the event loop. During the development process some issues related to the event loop/isolates topics it was getting clearer that you need to be careful. An interesting attempt to help you perform code safely with running your code in an isolate is class like [ConditionIsolateNoParallelFunctionCalls] (which with similar idea to the pool package (Pool class)) it controls whether or not a method is still synchronically and asynchronically executed and prevents another function to be executed.
All however unimaginable it may seem this app/library/whatever is going to work on all platforms including web with possible http backend server (global driver) simulation or proxy.
...
See below for more (first you need to one-time run commands to create db with more or less debug data), 
but now info: All debug users are created when "-a initDbStage=2", beter is to use 3, 4 - if you want max testing - tons of debug texts and waiting.
WARNING Because initDbStage cares for creating the db, first initDbStage can't be used with profileUser=X
WARNING - initDbStage can be run once for the default profile (profileUser param is ommitted) and once for each profileUser=X and when initDbStage is used with -a profileUser=4 or more then for each profileUser=X the param can be used with initDbStage only once! then you can use without the initDbStage - it is run normally with a profile chosen and a user logged in and active.
If initDbStage != null && profileUser is ommited first three users are created and it is a default profile
If initDbStage != null && profileUser is NOT ommited it currently must be > 3
If initDbStage is ommited default profileUser is used you are logged in in default way with the first three users attached to the app and two logged in and one active (id = 1 ?)
iF initDbStage is ommited you can use profileUser > 0 then you have one user attached to the app, the same logged in and the same active the id = profileUser 
Log in automatically in the debug mode to a chosen user, recommended two opitons:
  a: avoid the param entirely - you have three users registered id 1,3,2 in the app with one active (id=1?), probably two logged-in 
  b: is profileUser=4 id is registered, logged-in, and active on the front screen from the previous session.
examples:
  a: default: flutter run -d windows --debug -a debugging
  b: second recommended: flutter run -d windows --debug -a debugging -a profileUser=4
-----
